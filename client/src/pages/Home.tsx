    ...
    >
      ...
      <div 
        ...
        style={{ 
          ...
          touchAction: 'none' // Prevent manual scrolling on mobile
        }}
      >
        <div className="flex gap-2 sm:gap-4 md:gap-6 lg:gap-8" style={{ width: 'max-content' }}>
          {duplicatedPairs.map((pair, idx) => (
            <div
              key={`before-${idx}`}
              className="flex-shrink-0 relative w-[180px] h-[240px] sm:w-[220px] sm:h-[293px] md:w-[280px] md:h-[373px]"
            >
              <OptimizedImage
                src={pair.before}
              ...
              <div className="absolute top-2 left-2 sm:top-4 sm:left-4 z-10 bg-black/70 text-white text-[10px] sm:text-xs md:text-sm font-semibold px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg">
                {beforeLabel}
              </div>
            </div>
          ))}
        ...
        <div className="flex gap-2 sm:gap-4 md:gap-6 lg:gap-8" style={{ width: 'max-content' }}>
          {duplicatedPairs.map((pair, idx) => (
            ...
            >
              ...
              <div className="absolute top-2 right-2 sm:top-4 sm:right-4 z-20 bg-primary text-white text-[10px] sm:text-xs md:text-sm font-semibold px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg">
                {afterLabel}
              </div>
            </div>
          ))}
      ...
      <div
        className="absolute top-0 bottom-0 w-1 bg-white shadow-lg z-30 pointer-events-none h-[240px] sm:h-[293px] md:h-[373px]"
        style={{ 
          left: '50%', 
      ...
      >
        {/* Slider Handle */}
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-full bg-white shadow-xl flex items-center justify-center border-2 border-primary">
          <div className="flex gap-0.5 sm:gap-1">
            <svg className="w-2.5 h-2.5 sm:w-3 sm:h-3 md:w-4 md:h-4 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function Home() {
  ...
  // All variants (page2, page3, page4, page5) use the same mobile design: 3 reviews on mobile, 6 on desktop
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative min-h-0 sm:min-h-[50vh] lg:min-h-[55vh] overflow-hidden pt-4 pb-4 sm:pb-8 lg:pb-6 px-4 sm:px-6 bg-background">
        ...
        <div className="container relative z-10 pt-4 lg:pt-6">
          <div className="flex flex-col items-center text-center space-y-3 sm:space-y-4 max-w-4xl mx-auto">
            ...
            <p className="text-sm sm:text-lg md:text-xl text-muted-foreground max-w-3xl px-4 leading-relaxed">
              {parseMarkdownBold(isMobile ? (t("hero.subtitleMobile") || t("hero.subtitle") || "Turn your selfies into professional headshots with AI. One payment, instant delivery.") : (t("hero.subtitle") || "Turn your selfies into studio-quality headshots in minutes. Save hundreds of dollars and hours of your time."))}
            </p>
      ...
      {/* All variants (page2, page3, page4, page5) use the same design */}
      {howItWorksSection}

      {/* Why Choose Us Section - All variants use the same design */}
      {(
        <AnimatedSection>
      ...
      {/* As Seen On Section - All variants use the same design */}
      {(
